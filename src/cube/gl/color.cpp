...
# undef COLOR4
    ...
    template<typename T>
    inline void fill_hex_colors(char const* str, size_t len, T* colors, size_t n)
    {
        if (len < n || len % n != 0)
        {
            etc::log::error("Wrong string length", len, "for", n, "colors components in \"" + std::string(str) + "\".");
            return;
        }
        ...
        for (size_t col_idx = 0; col_idx < n; ++col_idx)
        {
            ...
            colors[col_idx] = PERCENT(T, static_cast<double>(col) / static_cast<double>(max_val));
            str += chars_by_colors;
        }
    }
